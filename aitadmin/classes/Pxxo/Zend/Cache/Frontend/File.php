 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Pxxo_Zend_Cache_Frontend_File extends Pxxo_Zend_Cache_Core
{
    ...
    public function __construct($options = array())
    {
        ...
        if (!isset($this->_specificOptions['master_file'])) {
            Pxxo_Zend_Cache::throwException('master_file option must be set');
        }
        clearstatcache();
        if (!($this->_masterFile_mtime = @filemtime($this->_specificOptions['master_file']))) {
            Pxxo_Zend_Cache::throwException('Unable to read master_file : '.$this->_specificOptions['master_file']);
        }
    }
