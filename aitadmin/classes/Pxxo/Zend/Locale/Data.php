...
class Pxxo_Zend_Locale_Data
{
    ...
    private static function _findRoute($locale, $path, $attribute, $value, &$temp)
    {
        ...
        if (empty(self::$_ldml[(string) $locale])) {
            ...
            if (!file_exists($filename)) {
                ...
                throw new Pxxo_Zend_Locale_Exception("Missing locale file '$filename' for '$locale' locale.");
            }
     ...
     *
     * @param string|Pxxo_Zend_Locale $locale Locale to check
     * @return string
     */
    private static function _checkLocale($locale)
    {
        ...
        if (!($locale = Pxxo_Zend_Locale::isLocale($locale))) {
            require_once 'Pxxo/Zend/Locale/Exception.php';
            throw new Pxxo_Zend_Locale_Exception("Locale ($locale) is a unknown locale");
        }
        return $locale;
    }
    ...
    public static function getList($locale, $path, $value = false)
    {
        ...
        if (isset(self::$_cache)) {
            ...
            $val = urlencode($val);
            $id = strtr('Pxxo_Zend_LocaleL_' . $locale . '_' . $path . '_' . $val, array('-' => '_', '%' => '_', '+' => '_'));
        ...
        switch(strtolower($path)) {
            ...
            default :
                require_once 'Pxxo/Zend/Locale/Exception.php';
                throw new Pxxo_Zend_Locale_Exception("Unknown list ($path) for parsing locale data.");
                break;
        }
    ...
    public static function getContent($locale, $path, $value = false)
    {
        ...
        if (isset(self::$_cache)) {
            ...
            $val = urlencode($val);
            $id = strtr('Pxxo_Zend_LocaleC_' . $locale . '_' . $path . '_' . $val, array('-' => '_', '%' => '_', '+' => '_'));
            if ($result = self::$_cache->load($id)) {
                return unserialize($result);
            }
        }
        ...
        switch(strtolower($path)) {
            ...
            default :
                require_once 'Pxxo/Zend/Locale/Exception.php';
                throw new Pxxo_Zend_Locale_Exception("Unknown detail ($path) for parsing locale data.");
                break;
        }
    ...
    /**
     * Set a cache for Pxxo_Zend_Locale_Data
     * 
     * @param Pxxo_Zend_Cache_Core $cache a cache frontend
     */
