 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Pxxo_Zend_Cache_Frontend_Page extends Pxxo_Zend_Cache_Core
{
    ...
    public function __construct($options = array())
    {
        ...
        if (isset($this->_specificOptions['http_conditional'])) {
            if ($this->_specificOptions['http_conditional']) {
                Pxxo_Zend_Cache::throwException('http_conditional is not implemented for the moment !');
            }
        }
    }
    ...
    {
        if (!is_array($options)) {
            Pxxo_Zend_Cache::throwException('default_options must be an array !');
        }
        foreach ($options as $key=>$value) {
            ...
            if (!isset($this->_specificOptions['default_options'][$key])) {
                Pxxo_Zend_Cache::throwException("unknown option [$key] !");
            } else {
    ...
    {
        if (!is_array($regexps)) {
            Pxxo_Zend_Cache::throwException('regexps option must be an array !');
        }
        foreach ($regexps as $regexp=>$conf) {
            if (!is_array($conf)) {
                Pxxo_Zend_Cache::throwException('regexps option must be an array of arrays !');
            }
