 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Pxxo_Zend_Cache_Backend_File extends Pxxo_Zend_Cache_Backend implements Pxxo_Zend_Cache_Backend_Interface
{
    ...
    public function __construct($options = array())
    {
        ...
        if (isset($this->_options['file_name_prefix'])) { // particular case for this option
            if (!preg_match('~^[\w]+$~', $this->_options['file_name_prefix'])) {
                Pxxo_Zend_Cache::throwException('Invalid file_name_prefix : must use only [a-zA-A0-9_]');
            }
        }
    }
    ...
    public function load($id, $doNotTestCacheValidity = false)
    {
        ...
        if ($this->_options['read_control']) {
            ...
            if ($hashData != $hashControl) {
                ...
                $this->_log('Pxxo_Zend_Cache_Backend_File::load() / read_control : stored hash and computed hash do not match');
                $this->_remove($file);
                return false;
            }
        }
    ...
    public function save($data, $id, $tags = array(), $specificLifetime = false)
    {
        if ((!is_dir($this->_options['cache_dir'])) or (!is_writable($this->_options['cache_dir']))) {
            $this->_log("Pxxo_Zend_Cache_Backend_File::save() : cache_dir doesn't exist or is not writable");
        }
    ...
    private function _remove($file)
    {
        if (!@unlink($file)) {
            ...
            $this->_log("Pxxo_Zend_Cache_Backend_File::_remove() : we can't remove $file => we are going to try to invalidate it");
            return false;
        }
        return true;
    }
     ...
     * @return boolean true if no problem
     */
    private function _clean($dir, $mode = Pxxo_Zend_Cache::CLEANING_MODE_ALL, $tags = array())
    {
        if (!is_dir($dir)) {
            return false;
        }
    ...
    private function _hash($data, $controlType)
    {
        ...
        default:
            Pxxo_Zend_Cache::throwException("Incorrect hash function : $controlType");
        }
    }
