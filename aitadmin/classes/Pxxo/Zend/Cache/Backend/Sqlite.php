 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Pxxo_Zend_Cache_Backend_Sqlite extends Pxxo_Zend_Cache_Backend implements Pxxo_Zend_Cache_Backend_Interface
{
    ...
    public function __construct($options = array())
    {
        ...
        if (is_null($this->_options['cache_db_complete_path'])) {
            Pxxo_Zend_Cache::throwException('cache_db_complete_path option has to set');
        }
        $this->_db = @sqlite_open($this->_options['cache_db_complete_path']);
        if (!($this->_db)) {
            Pxxo_Zend_Cache::throwException("Impossible to open " . $this->_options['cache_db_complete_path'] . " cache DB file");
        }
    }
    ...
    public function save($data, $id, $tags = array(), $specificLifetime = false)
    {
        if (!$this->_checkStructureVersion()) {
            ...
            if (!$this->_checkStructureVersion()) {
                Pxxo_Zend_Cache::throwException("Impossible to build cache structure in " . $this->_options['cache_db_complete_path']);
            }
        }
        ...
        if (!$res) {
            $this->_log("Pxxo_Zend_Cache_Backend_Sqlite::save() : impossible to store the cache id=$id");
            return false;
        }
        $res = true;
        foreach ($tags as $tag) {
    ...
    private function _registerTag($id, $tag) {
        ...
        if (!$res) {
            $this->_log("Pxxo_Zend_Cache_Backend_Sqlite::_registerTag() : impossible to register tag=$tag on id=$id");
            return false;
        }
        return true;
    }
    ...
    private function _checkStructureVersion()
    {
        ...
        if (((int) $row['num']) != 1) {
            // old cache structure
            $this->_log('Pxxo_Zend_Cache_Backend_Sqlite::_checkStructureVersion() : old cache structure version detected => the cache is going to be dropped');
            return false;
        }
        return true;
    }
     ...
     * @return boolean true if no problem
     */
    private function _clean($mode = Pxxo_Zend_Cache::CLEANING_MODE_ALL, $tags = array())
    {
        ...
        if ($mode==Pxxo_Zend_Cache::CLEANING_MODE_OLD) {
            $mktime = time();
