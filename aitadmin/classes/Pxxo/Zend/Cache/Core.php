...
class Pxxo_Zend_Cache_Core
{
    ...
    {
        if (!is_array($options)) {
            Pxxo_Zend_Cache::throwException('Options parameter must be an array');
        }
    ...
    public function setBackend($backendObject)
    {
        if (!is_object($backendObject)) {
            Pxxo_Zend_Cache::throwException('Incorrect backend object !');
        }
        $this->_backend= $backendObject;
        // some options (listed in $_directivesList) have to be given
        // to the backend too (even if they are not "backend specific")
        $directives = array();
        foreach (Pxxo_Zend_Cache_Core::$_directivesList as $directive) {
    ...
    public function setOption($name, $value)
    {
        ...
        Pxxo_Zend_Cache::throwException("Incorrect option name : $name");
    }
    ...
    public function save($data, $id = null, $tags = array(), $specificLifetime = false)
    {
        ...
        if ($this->_options['automatic_serialization']) {
            ...
            if (!is_string($data)) {
                Pxxo_Zend_Cache::throwException("Datas must be string or set automatic_serialization = true");
            }
        }
        // automatic cleaning
        if ($this->_options['automatic_cleaning_factor'] > 0) {
            ...
            if ($rand==1) {
                if ($this->_backend->isAutomaticCleaningAvailable()) {
                    $this->clean(Pxxo_Zend_Cache::CLEANING_MODE_OLD);
                } else {
                    $this->_log('Pxxo_Zend_Cache_Core::save() / automatic cleaning is not available with this backend');
                }
            }
        }
        ...
        if (!$result) {
            ...
            if ($this->_options['logging']) {
                $this->_log("Pxxo_Zend_Cache_Core::save() : impossible to save cache (id=$id)");
            }
            $this->remove($id);
            return false;
        }
        if ($this->_options['write_control']) {
            ...
            if ($data!=$data2) {
                $this->_log('Pxxo_Zend_Cache_Core::save() / write_control : written and read data do not match');
                $this->remove($id);
                return false;
            }
        }
    ...
    public function clean($mode = 'all', $tags = array())
    {
        ...
        if (!in_array($mode, array(Pxxo_Zend_Cache::CLEANING_MODE_ALL, Pxxo_Zend_Cache::CLEANING_MODE_OLD, Pxxo_Zend_Cache::CLEANING_MODE_MATCHING_TAG, Pxxo_Zend_Cache::CLEANING_MODE_NOT_MATCHING_TAG))) {
            Pxxo_Zend_Cache::throwException('Invalid cleaning mode');
        }
        self::_validateTagsArray($tags);
    ...
    {
        if (!is_string($string)) {
            Pxxo_Zend_Cache::throwException('Invalid id or tag : must be a string');
        }
        if (substr($string, 0, 9) == 'internal-') {
            Pxxo_Zend_Cache::throwException('"internal-*" ids or tags are reserved');
        }
        if (!preg_match('~^[\w]+$~', $string)) {
            Pxxo_Zend_Cache::throwException('Invalid id or tag : must use only [a-zA-Z0-9_]');
        }
    }
    ...
    {
        if (!is_array($tags)) {
            Pxxo_Zend_Cache::throwException('Invalid tags array : must be an array');
        }
        foreach($tags as $tag) {
    ...
    /**
     * Make sure if we enable logging that the Pxxo_Zend_Log class
     * is available.
     * Create a default log object if none is set.
     *
     * @return void
     * @throws Pxxo_Zend_Cache_Exception
     */
    protected function _loggerSanity()
    {
        ...
        try {
            require_once 'Pxxo/Zend/Loader.php';
            Pxxo_Zend_Loader::loadClass('Pxxo_Zend_Log');
        } catch (Pxxo_Zend_Exception $e) {
            Pxxo_Zend_Cache::throwException('Logging feature is enabled but the Pxxo_Zend_Log class is not available');
        }
        if (isset($this->_options['logger']) && $this->_options['logger'] instanceof Pxxo_Zend_Log) {
            return;
        }
        // Create a default logger to the standard output stream
        Pxxo_Zend_Loader::loadClass('Pxxo_Zend_Log_Writer_Stream');
        $logger = new Pxxo_Zend_Log(new Pxxo_Zend_Log_Writer_Stream('php://output'));
    ...
    protected function _log($message, $priority = 4)
    {
        ...
        if (!(isset($this->_options['logger']) || $this->_options['logger'] instanceof Pxxo_Zend_Log)) {
            Pxxo_Zend_Cache::throwException('Logging is enabled but logger is not set');
        }
