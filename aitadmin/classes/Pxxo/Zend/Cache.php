 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class Pxxo_Zend_Cache
{
    ...
    public static function factory($frontend, $backend, $frontendOptions = array(), $backendOptions = array())
    {
        ...
        if (!in_array($backend, Pxxo_Zend_Cache::$availableBackends)) {
            self::throwException("Incorrect backend ($backend)");
        }

        // For perfs reasons, with frontend == 'Core', we can interact with the Core itself
        $frontendClass = 'Pxxo_Zend_Cache_' . ($frontend != 'Core' ? 'Frontend_' : '') . $frontend;

        $backendClass = 'Pxxo_Zend_Cache_Backend_' . $backend;

        // For perfs reasons, we do not use the Pxxo_Zend_Loader::loadClass() method
     ...
     * Throw an exception
     *
     * Note : for perf reasons, the "load" of Pxxo/Zend/Cache/Exception is dynamic
     */
    public static function throwException($msg)
    {
        // For perfs reasons, we use this dynamic inclusion
        require_once 'Pxxo/Zend/Cache/Exception.php';
        throw new Pxxo_Zend_Cache_Exception($msg);
    }
